window.onload=function () {
    ...
    let listnave = document.querySelectorAll(".rownave .list-nave");
    let hidden = document.querySelectorAll(".hidden a")
    // console.log(listnave, hidden)
    // let index=0
    hidden[0].classList.add("hot")
    listnave[0].style.display="block";
    listnave[1].style.display="none";
    listnave[2].style.display="none";
    console.log(listnave[2])
    for (let i = 0; i < hidden.length; i++) {
        hidden[i].onclick = function () {
            for (let j = 0; j < listnave.length; j++) {
                listnave[j].style.display = "none"
                hidden[j].className="";
            }
            listnave[i].style.display = "block";
            hidden[i].className="hot";
            // console.log(hidden);
        }
